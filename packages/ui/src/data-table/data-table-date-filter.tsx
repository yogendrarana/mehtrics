...
export function DataTableDateFilter<TData>({ column, title, multiple }: DataTableDateFilterProps<TData>) {
  ...
  const label = React.useMemo(() => {
    ...
    const dateText = hasSelectedDate && date ? formatDate(date) : "Select date";
    ...
    <Popover>
      ...
      <PopoverContent className="w-auto p-0" align="start">
        {multiple ? (
          ...
          <Calendar autoFocus mode="single" selected={!getIsDateRange(selectedDates) ? selectedDates[0] : undefined} onSelect={onSelect} />
        )}
      </PopoverContent>
    </Popover>
  );
